        ...
        {loading
          ? Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="flex-shrink-0 w-[68px]">
                <div className="w-[68px] h-[102px] rounded-sm bg-surface-hover animate-pulse" />
                <div className="mt-2 h-2.5 w-10 rounded bg-surface-hover animate-pulse" />
                <div className="mt-1 h-2 w-8 rounded bg-surface-hover animate-pulse" />
              </div>
            ))
          : books.map((book) => (
              <Link
